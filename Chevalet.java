...
public class Chevalet
{
    ...
    // Constructeur : Créer un chevalet avec un nom. Le chevalet et cide est le score est 0
    ...
    public boolean piocherLettre(Sac leSac)
    {
        ...
        // pour chaque case du chevaket, si elle est vide, on pioche une lettre au hasard dans le sac 
        for (i=0; i<7; i++)
        {
            ...
            else
            {
                for (j=0; j<7; j++)
                {
                    if (this.mesLettres[j].symbole == LettresAChanger.charAt(i))
                    {
                        ...
                        break;
                        // le break sert à ce toutes les mêmes lettres dans le chevalet ne soient pas changées en même temps
                    }
                }
            }
        }
    ...
    // retourne une classe/structure ChoixJouer, avec toute les données nécessaires pour jouer un coup
    public ChoixJouer ChoixCoup()
    {
        ...
        if (leChoix.choix == 1)
        {
            ...
            // on traduit les direction en dx et dy, qu'on va entrer dans les autres méthodes, 
            // pour ne pas avoir à refaire la disjontion de cas à  chaque foid
            if (direction == 'N')
            {
                leChoix.dx = 0;
                leChoix.dy = -1;
    ...
    // méthode qui permet de jouer la premier coup de la partie (qui a des règles différentes des autres coup de la partie)
    public Plateau JouerPremierCoup(Plateau lePlateau, Sac leSac)
    {
        ...
        while (!(coupValide))
        {
            ...
            // si le joueur veut remplacer ses lettres
            if (leChoix.choix == 2)
            {
